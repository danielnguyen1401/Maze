...
public class DivisionMazeGenerator : BasicMazeGenerator
{
    ...
    private struct IntRect
    {
        ...
        {
            return $"[IntRect {left}-{right} {bottom}-{top}]";
        }
    }
    ...
    private void divideVertical(IntRect rect)
    {
        ...
        rectsToDivide.Enqueue(
            new IntRect() {left = rect.left, right = divCol + 1, bottom = rect.bottom, top = rect.top});
        rectsToDivide.Enqueue(new IntRect()
        {
            left = divCol + 1,
            right = rect.right,
            bottom = rect.bottom,
